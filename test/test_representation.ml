...
let test_inc_1 =
  let result = skew_to_int (inc s1) in
  let desired = (skew_to_int s1 + 1) in
  Alcotest.test_case "smallest weight is 2 " `Quick (fun () ->
    Alcotest.(check int) "same result" desired result)
  
    let test_inc_2 =
      let result = skew_to_int (inc s2) in
      let desired = (skew_to_int s2 + 1) in
      Alcotest.test_case "2 smallest weights are 1 " `Quick (fun () ->
        Alcotest.(check int) "same result" desired result)
...
let () =
    ...
    run "Skew"
      [
        ("skew_to_int", [ test_skew_to_int_1; test_skew_to_int_2; test_skew_to_int_3; test_skew_to_int_4 ]);
        ("incr", [ test_inc_1; test_inc_2 ])
      ]
